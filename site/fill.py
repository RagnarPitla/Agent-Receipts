...
PROOF FAIL 2 skill(s) named in the ledger are not on the agent
  ...
  MISSING: weak-skill
      &lt;- push reported success; the component does not exist</pre>
          ...
          ledger, not an example, and this is what it says today. Ten gates are closed by a
          command. Two are open, so the run exits 1 and the project is not done.</p>
...
<pre><span class="prompt">$</span> receipts check GATES.md --approve
  ...
  NOT MET</pre>
          ...
          <div class="stat-row">
            <div class="stat"><b>35</b><span>tests, all passing, no network</span></div>
          ...
          <p class="muted">Every number above comes from a file you can read. 35 is what
...
FAQ = [
    ...
    ("Can I try it without a tenant?",
     ...
     <p>The full test suite - 35 tests - runs with no network and no credentials, which is also why
    ...
    ("What is not finished?",
     """<p>The engine, the eight probes, the lint rules and the 35 tests all work today. What does
